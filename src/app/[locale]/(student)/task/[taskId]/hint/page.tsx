...
function HintContent() {
  ...
  const t = useTranslations("taskHint");
  ...
  if (!hint) return <div className="text-center py-20 text-text-dim">{t("loadFailed")}</div>;
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-amber/10 border border-amber/20 rounded-2xl p-5 mb-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-amber mb-1">{t("title")}</h1>
            <p className="text-sm text-text-dim">
              {t("intro")}
              <span className="font-semibold text-amber"> {t("warning")}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      {hint.steps.length > 0 && (
        <div className="bg-white rounded-2xl border border-border p-5 mb-4">
          <h3 className="text-sm font-semibold text-accent mb-3">{t("stepsTitle")}</h3>
      ...
      {hint.keyPoints.length > 0 && (
        <div className="bg-white rounded-2xl border border-amber/20 p-5 mb-4">
          <h3 className="text-sm font-semibold text-amber mb-3">{t("tipsTitle")}</h3>
      ...
      <div className="bg-white rounded-2xl border border-border p-5 mb-6">
        <h3 className="text-sm font-semibold text-cyan mb-3">{t("templateTitle")}</h3>
      ...
      <div className="bg-white rounded-2xl border border-border p-6">
        <h3 className="font-bold mb-4 text-text">{t("resubmitTitle")}</h3>
        <textarea
          value={content}
...
function HintFallback() {
  ...
  return (
    <div className="flex items-center justify-center py-20">
      <div className="text-text-dim">{t("generating")}</div>
    </div>
  );
}
...
export default function HintPage() {
  return (
    <Suspense fallback={<HintFallback />}>
      <HintContent />
    </Suspense>
  );
}
