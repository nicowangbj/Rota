...
const PHASE_THEMES = [
  { main: "#6366f1", light: "#eef2ff", gradient: "from-indigo-500 to-purple-500", icon: "📖", labelKey: "phaseTheme1" },
  { main: "#06b6d4", light: "#ecfeff", gradient: "from-cyan-500 to-blue-500", icon: "🔍", labelKey: "phaseTheme2" },
  { main: "#22c55e", light: "#f0fdf4", gradient: "from-green-500 to-emerald-500", icon: "🧪", labelKey: "phaseTheme3" },
  { main: "#f59e0b", light: "#fffbeb", gradient: "from-amber-500 to-orange-500", icon: "📊", labelKey: "phaseTheme4" },
  { main: "#f43f5e", light: "#fff1f2", gradient: "from-rose-500 to-pink-500", icon: "✍️", labelKey: "phaseTheme5" },
    ...
    <div className="flex gap-6 max-w-[1300px] mx-auto">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-text-dim">
                  {t("progress", { current: currentWeek, total: totalWeeks, done: completedTasks, all: totalTasks })}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={() => router.push(`/${locale}/journal?projectId=${project.id}`)}
              ...
              <button
                onClick={() => router.push(`/${locale}/adjust?projectId=${project.id}`)}
                className="px-3.5 py-2 bg-surface2 hover:bg-accent/10 text-text-dim hover:text-accent text-sm font-medium rounded-xl transition-colors"
              >
                {t("adjustBtn")}
              </button>
                  ...
                  <div className={`flex items-start gap-4 ${isLeft ? "mr-auto pr-12" : "ml-auto pl-12"}`} style={{ maxWidth: "75%" }}>
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        {isLocked && (
                          <span className="text-[10px] text-text-muted ml-auto">{t("statusLocked")}</span>
                        )}
                      </div>
            ...
            <div className="flex justify-center">
              ...
              >
                ...
                <span className={`text-[10px] font-semibold mt-0.5 ${progress === 100 ? "text-amber-600" : "text-text-muted"}`}>
                  {progress === 100 ? t("finishDone") : t("finishPending")}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="w-[340px] shrink-0 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-border overflow-hidden">
          ...
          <div className="p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold text-text">{t("overviewTitle")}</h2>
              <span className="text-xs text-text-muted">{t("phaseCount", { n: project.phases.length })}</span>
            </div>
                ...
                <div className="bg-white/80 rounded-lg rounded-tl-sm px-3 py-2">
                  <p className="text-[11px] text-text-dim leading-relaxed">
                    {activePhase?.status === "active"
                      ? t("activePhaseMsg", { name: activePhase.name })
                      : t("welcomeMsg")}
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-accent/5 rounded-xl p-2.5 text-center">
                ...
                <p className="text-[10px] text-text-muted">{t("statDone")}</p>
              </div>
              <div className="bg-cyan/5 rounded-xl p-2.5 text-center">
                <p className="text-lg font-bold text-cyan">{totalTasks - completedTasks}</p>
                <p className="text-[10px] text-text-muted">{t("statPending")}</p>
              </div>
              <div className="bg-green/5 rounded-xl p-2.5 text-center">
                <p className="text-lg font-bold text-green">{t("weekLabel", { n: currentWeek })}</p>
                <p className="text-[10px] text-text-muted">{t("statCurrentWeek")}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-border overflow-hidden">
            ...
            <div className="p-5">
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-text-muted">
                    {t("weekRange", { start: selectedPhaseData.startWeek ?? "", end: selectedPhaseData.endWeek ?? "" })} · {t("taskCount", { n: selectedPhaseData.tasks.length })}
                  </p>
                </div>
              </div>
              ...
              {selectedPhaseData.goal && (
                ...
                >
                  <span className="font-semibold">{t("goalLabel")}</span>{selectedPhaseData.goal}
                </div>
              )}
                ...
                {selectedPhaseData.tasks.map((task) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (!isTaskLocked) {
                          router.push(`/${locale}/task/${task.id}?projectId=${project.id}`);
                        }
                      }}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {task.weekNumber && (
                          <p className="text-[10px] text-text-muted mt-0.5">{t("taskWeek", { n: task.weekNumber ?? "" })}</p>
                        )}
                      </div>
...
function MapFallback() {
  ...
  return (
    <div className="flex items-center justify-center py-20">
      <div className="text-text-dim">{t("loadingSpinner")}</div>
    </div>
  );
}
