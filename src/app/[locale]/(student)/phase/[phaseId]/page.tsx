...
function PhaseContent() {
  ...
  if (!phase) return <div className="text-center py-20 text-text-dim">{t("notFound")}</div>;
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
            ...
            <div className="p-6">
              ...
              {phase.goal && (
                <div className="bg-surface2 rounded-xl p-3 mt-3">
                  <span className="text-xs font-semibold text-text-muted">{t("goalLabel")}</span>
          ...
          <h2 className="text-lg font-bold mb-4 text-text">{t("tasksTitle")}</h2>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {isActive && (
                        <span className="px-2.5 py-1 bg-accent/10 text-accent text-xs font-medium rounded-full">{t("statusActive")}</span>
                      )}
                      {isDone && (
                        <span className="px-2.5 py-1 bg-green/10 text-green text-xs font-medium rounded-full">{t("statusDone")}</span>
                      )}
                      {isLocked && (
                        <span className="text-xs text-text-muted">{t("statusLocked")}</span>
                      )}
                      {!isLocked && (
              ...
              <div className="h-96 border-t border-border pt-4">
                <ChatWindow
                  ...
                  initialMessages={[
                    {
                      role: "assistant",
                      content: t("initialMsg", { phaseName: phase.name, goal: phase.goal || t("defaultGoal") }),
                    },
                  ]}
                />
              </div>
